  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'remove':
        ...
        let pp = 'https://telegra.ph/file/46f349ad6b64839cd401b.jpg'
            try {
              ...
              let wm = global.botwm
              let wel = await (await fetch(pp)).buffer()
              let lea = await (await fetch(pp)).buffer()              
              conn.send2ButtonLoc(jid, action === 'add' ? wel : lea, text, action === 'add' ? '*© fahril*' : '*© fahril*', '⋮☰ Menu', '#menu', 'Info Grup', '.infogc', false, {
                contextInfo: {
                  mentionedJid: [user]
                }
              })
