	...
	return new Promise((resolve, reject) => {
		fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
			...
			.then(res => {
				...
				data.splice(2, 1)
				if (data.length == 0) return resolve({ creator: '@fahril - @neoxr - Wildan Izzudin & @ariffb.id - Ariffb', status: false })
				resolve({ creator: '@fahril - @neoxr - Wildan Izzudin & @ariffb.id - Ariffb', status: true, data })
			}).catch(reject)
	})
}